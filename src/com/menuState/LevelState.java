...
import com.sprite.Bricks.NormalBrick;
import com.sprite.Bricks.UnBreakBricks;
import com.sprite.Bricks.multihit;
...
//	Constructor
	public LevelState(StateManager sm, int rows, int columns) {
		super(sm);
		try {
			Background bg = new Background("/Resources/lvlSelect.jpg", 1);
			bg.setVector(0, 0);
		...
		for (int i = 0; i < this.getRows(); i++) {
			for (int j = 0; j < this.getColumns(); j++) {
				...
				case 2: // multi-hit brick
					bricks[k] = new multihit(j * Brick.BRICK_WIDTH + Brick.BRICK_X_OFFSET,
							i * Brick.BRICK_HEIGHT + Brick.BRICK_Y_OFFSET, 'n');
					break;
				...
				case 3: // unbreakable brick
					bricks[k] = new UnBreakBricks(j * Brick.BRICK_WIDTH + Brick.BRICK_X_OFFSET,
							i * Brick.BRICK_HEIGHT + Brick.BRICK_Y_OFFSET, 'n');
	...
	private void drawObjects(Graphics2D g2d) {
		...
		g2d.drawString(difficulty[options.currDiff], 250, 30);
		...
		for (int i = 0; i < numOfBricks; i++) {
			if ((ball.getRect()).intersects(bricks[i].getRect())) {
				...
				if (!bricks[i].isDestroyed()) {
					...
					if (!(bricks[i] instanceof UnBreakBricks)) {
						if (bricks[i] instanceof NormalBrick) {

							Brick.breaks(this.drops);
							this.score += addScoreValue;

						} else if (bricks[i] instanceof multihit) {

							multihit multihit = (multihit) bricks[i];

							multihit.setHitCount(multihit.getHitCount() + 1);

							// update image if cracked
							if (multihit.getHitCount() == multihit.getMaxHit() - 1) {
								multihit.crack();
							}

							// multi-hit brick is destroyed if max hit is reached
							else if (multihit.getHitCount() == multihit.getMaxHit()) {
								Brick.breaks(this.drops);
								this.score += addScoreValue;
							}
